...
exports.createMeta = async (req, res) => {
    try {
        const { pageName, metaTitle, pageSlug, metaDescription, metaKeyword, canonicalLink, faqSchema } = req.body;
        const newMeta = new Meta({ pageName, metaTitle, metaDescription, metaKeyword, pageSlug, canonicalLink, faqSchema });
        await newMeta.save();
...
exports.updateMeta = async (req, res) => {
    try {
        const { pageName, metaTitle, metaDescription, pageSlug, metaKeyword, canonicalLink, faqSchema } = req.body;
        const updatedMeta = await Meta.findByIdAndUpdate(
            req.params.id,
            { pageName, metaTitle, metaDescription, metaKeyword, pageSlug, canonicalLink, faqSchema },
